...
def wpisaneHaslo():
    ...
    matched_words = [
        string for string in headlines
...
def scrape_definition(word):
    ...
    try:
        ...
        znaczenie = soup.find("b", string="znaczenie:")

        if not znaczenie:
            return "Brak definicji"

        definition_p = znaczenie.find_parent("p").find_next_sibling("p")

        if not definition_p:
            return "Brak definicji"

        definition = " ".join(definition_p.get_text().split())

        return definition

    except Exception as e:
        app.logger.info(e)
        return "Błąd pobierania definicji"

    matched_words = [string for string in headlines if re.match(regex, string)]
